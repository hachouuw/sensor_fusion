...
class serialPlot:
    def __init__(self, serialPort, serialBaud, plotLength, dataNumBytes):
        ...
        # self.dataPlot = collections.deque([0] * plotLength, maxlen=plotLength)
        self.data = np.array([])
        self.isRun = True
    ...
    # def rescale_inp(inp,MIN=-1000,MAX=328):
    #     return 2 * ( (inp - MIN) / (MAX - MIN) - .5) * (3./2.)
...
def main():
    ...
    for n in range(0,200): #200
        # s.grabData()     #or append it to your other data or whatever
        s.rescale_inp(s.grabData())
        time.sleep(1./50.)
